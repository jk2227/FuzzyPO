...
class FuzzyPORecommender:
    def __init__(self, articles, json_str = None):
        ...
        self.N = len(self.knowledge.UniqueProcesses)
        self.random = Random()
    ...
    def color_gain(self, id, alpha=1.0):
        color_easier = sum(1 for i in self.easiers[id] if self.color[i] == -1)
        color_harder = sum(1 for i in self.harders[id] if self.color[i] == -1)
    ...
    def request(self):
        ...
        #response_history_stats = {True:0, False:0}
        #for r in self.response_history:
        #    if r == True or r == False:
        #        response_history_stats[r] += 1
    ...
    def color_node(self, id, res):
        if res == True:
            self.color[id] = 1
            for i in range(self.N):
                if self.knowledge.easier_graph[i][id] and self.color[i] == -1:
                    self.color[i] = 1
        elif res == False:
            self.color[id] = 0
            for i in range(self.N):
                if self.knowledge.easier_graph[id][i] and self.color[i] == -1:
                    self.color[i] = 0
